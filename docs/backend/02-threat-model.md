...
|---|---|---|---|
| ~~RR-7~~ | ~~The general per-principal rate limiter is configured and keyed but not installed~~ | **CLOSED in the closure phase.** The limiter is installed and enforcing: charged per principal in the request extractors, with a coarse per-address ceiling ahead of authentication. Measured before and after on a live instance — see `RATE_LIMIT_ARCHITECTURE.md`. TH-48 is bounded accordingly | — |
| ~~RR-8~~ | ~~A control existing in configuration but not in code~~ | **CLOSED.** All eleven limits are read from the environment, and a quota of `0` is refused at startup rather than read as "unlimited" | — |
| RR-9 | Rate limiting is enforced **per process**; there is no cross-instance enforcement | The deployment is a single API container, and adding distributed state for a distribution that does not exist buys nothing | One narrow `RateLimiter` trait; a Redis implementation is two methods. **Running a second instance requires it first** |
