...
/// behaviour, and a feature flag key is not a capability — see the note in
/// `modules::settings`.
pub async fn info(state: &AppState, _principal: &Principal) -> AppResult<SystemInfoResponse> {
    let initialized = repo::is_initialized(&state.db).await?;
    let enabled_features = repo::enabled_feature_flag_keys(&state.db).await?;
